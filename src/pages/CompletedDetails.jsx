...
function CompletedDetails() {
  ...
  const align = () => {
    const title = path[3]
    if (title === "alquran" || title === "hadits") return "center"
    return "left"
  }
  ...
  const { subjectDetails, isLoading: isLoadingSubject } = useSelector(
  ...
  const generusCount =
    countList?.countRoles?.find((o) => o._id === "GENERUS")?.total || 0.0001
  const findCompleteCount = (target) => {
    return (
      completedTargets?.targetsCompleted?.find((o) => o.target === target)
        ?.count || 0
    )
  }
  ...
  const style = (target) => {
    ...
    style.backgroundColor =
      onClick === target ? "grey" : colorScale(scale).hex()
    ...
    <>
      ...
      <Stack direction='row' spacing={0.5}>
        <Chip
          ...
          label={
            <Typography sx={{ fontSize: 10 }}>
              Ds: {capitalize.words(ds || "semuda ds")}
            </Typography>
          }
          color='info'
        />
        <Chip
          ...
          label={
            <Typography sx={{ fontSize: 10 }}>
              Klp: {capitalize.words(klp || "semuda klp")}
            </Typography>
          }
          color='info'
        />
      ...
      <Box textAlign={align()} pt={1}>
        {isLoadingSubject ? (
          ...
          subjectTargets
            .slice(chunkSize * (page - 1), chunkSize * page)
            .map((target) => (
              <Chip
                variant='solid'
                key={target}
                ...
                label={
                  <Typography sx={{ fontSize: 10 }}>
                    ...
                    {target === onClick
                      ? (
                          (findCompleteCount(target) / generusCount) *
                          100
                        ).toFixed(1) + "%"
                      : target}{" "}
                  </Typography>
